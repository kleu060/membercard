...
import { Calendar, MapPin, Mail, Phone, Briefcase, GraduationCap, Award, Settings, Save, X, FileText, Plus, Edit, Trash2 } from 'lucide-react';
...
export default function JobProfilePage() {
  ...
  const fetchJobProfile = async () => {
    try {
      const response = await fetch('/api/job-profile');
      if (response.ok) {
        const data = await response.json();
        setJobProfile(data);
      } else {
        // If profile doesn't exist, create one
        const createResponse = await fetch('/api/job-profile', {
          method: 'POST',
          headers: {
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="text-sm font-medium">上传简历</label>
