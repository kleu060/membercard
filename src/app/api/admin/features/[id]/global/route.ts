...
// Mock features data (same as in other feature routes)
const mockFeatures = [
  {
    id: 'booking-system',
    name: '預約系統',
...
export async function PUT(
  ...
  try {
    const { globalEnabled } = await request.json();
    ...
    console.error('Error updating feature global status:', error);
    return NextResponse.json(
      { error: 'Failed to update feature global status' },
      { status: 500 }
    );
  }
}